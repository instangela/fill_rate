-- Taken from https://raw.githubusercontent.com/dbt-labs/redshift/main/macros/unload.sql
-- Modified to support custom SQL query
...
{% macro unload_table(schema,
  ...
  {% if where %}
  UNLOAD ($$SELECT * FROM "{{ schema }}"."{{ table }}" WHERE {{ where }}$$)
  {% else %}
