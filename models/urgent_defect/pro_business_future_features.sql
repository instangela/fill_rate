SELECT ds, ID_worker_id, ID_business_id, MC_business_region, RV_INT_FUTURE_running_assigneds_with_business, RV_INT_FUTURE_running_unassigneds_with_business FROM (
    SELECT
        ...
        , COALESCE(RV_INT_FUTURE_running_assigneds_with_business, 0) AS RV_INT_FUTURE_running_assigneds_with_business
        , COALESCE(RV_INT_FUTURE_running_unassigneds_with_business, 0) AS RV_INT_FUTURE_running_unassigneds_with_business
    FROM (
    SELECT
        psi.ID_worker_id
...
)  WHERE rn_worker > 0 AND rn_worker_business = 1
